document.getElementById("register-form").addEventListener("submit", async function (event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            localStorage.setItem('registerSupplierToken', result.token); // Save JWT token in local storage
            alert('Registration successful!'); // Notify the user
            window.location.href = "../login/loginSupplier.html"; // Redirect to login page
        } else {
